...
# ---------------------------------------------------------------------------
# SECTOR ROUTING & INTELLIGENCE COMPILERS (SYNCHRONOUS WORKERS)
# ---------------------------------------------------------------------------
def compile_equities_options_intel(engine, ticker):
    """The 'Gold Standard' Options Sniper Architecture (TastyTrade + UW Hybrid)."""
    try:
        ...
        # Pulling 50D SMA for regime context
        ...
        # Synthetic IVR / PoP generation based on regime for rapid terminal output
        ...
        return "⚠️ Telemetry unavailable. Verify asset ticker or API limits.", 0xe74c3c
...
def compile_crypto_intel(engine, ticker):
    ...
    try:
        ...
        return "⚠️ Crypto Telemetry unavailable.", 0xe74c3c

def compile_income_intel(engine, ticker):
    """Dividend Wheel Cash-Secured Put Logic (SeekingAlpha Hybrid)."""
    try:
        price_data = engine._execute_query("price", {"symbol": ticker})
        spot = float(price_data.get("price", 0.0))
        if spot == 0: return None, None
        ...
        return "⚠️ Income Telemetry unavailable.", 0xe74c3c

def process_query_routing(engine, ticker):
    """Synchronous routing function to be pushed to a thread."""
...
# ---------------------------------------------------------------------------
# SLASH COMMAND INTERFACE & FAIL-SAFES
# ---------------------------------------------------------------------------
...
async def query_asset(interaction: discord.Interaction, ticker: str):
    # Ephemeral defers so NO ONE else in the channel sees the "Thinking..." or output
    await interaction.response.defer(ephemeral=True, thinking=True)
    ticker = ticker.upper().strip()
    logger.info(f"Command /query triggered for {ticker} by {interaction.user}")
    
    try:
        # Offload the blocking API requests to a background thread
        ...
        # Sends the payload silently to the querying user only
        await interaction.followup.send(embed=embed, ephemeral=True)

    except Exception as e:
        logger.error(f"Critical failure during /query execution: {e}")
        await interaction.followup.send("⚠️ Ecosystem structural fault. The API layer timed out. Please try again.", ephemeral=True)
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    ...
    if not interaction.response.is_done():
        await interaction.response.send_message("⚠️ An unexpected structural error occurred processing this command.", ephemeral=True)
