...
def mock_requests_get(url, *args, **kwargs):
    ...
    if "api.twelvedata.com" in url:
        ...
        if "quote" in url: return MockResponse({"close": "45.00"})
        if "time_series" in url: return MockResponse({"values": [{"close": "7.50"} for _ in range(25)]})
...
def mock_requests_post(url, *args, **kwargs):
    ...
    if "generativelanguage.googleapis.com" in url:
        return MockResponse({"candidates": [{"content": {"parts": [{"text": '{"discord_embed_brief": "WARGAME SIMULATION VALID."}'}]}}]})
